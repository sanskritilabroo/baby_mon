...
if not hasattr(np, "float"):
    np.float = float
float32 = np.float32   # alias for old code
...
def process (input_image, params, model_params):
    ...
    for m in range(len(multiplier)):
        ...
        input_img = np.transpose(imageToTest_padded[:,:,:,np.newaxis].astype(np.float32), (3,0,1,2))
...
class VideoProcessorSK(VideoProcessor):
    ...
    def close(self):
        getattr(self, "svid", None) and getattr(self, "svid", None) and getattr(self, "svid", None) and getattr(self, "svid", None) and getattr(self, "svid", None) and self.svid.close()
        getattr(self, "vid", None) and getattr(self, "vid", None) and getattr(self, "vid", None) and getattr(self, "vid", None) and getattr(self, "vid", None) and self.vid.close()
...
for ivid,vid in enumerate(videos):
    ...
    if os.path.isfile(os.path.join(input_path,vname + '_openposeLabeled.mp4')):
        ...
        processed_frames = []  # store frames during processing
        ...
        for index in tqdm(range(nframes)):
            ...
            try:
                ...
                clip.save_frame(frame)  # optional
                processed_frames.append(frame)  # store for video

            except Exception as e:
                print(f"Frame {index} failed: {e}")

        # combine into video
        from skvideo.io import FFmpegWriter
        output_video_path = os.path.join(input_path, vname + '_openposeLabeled.mp4')
        writer = FFmpegWriter(output_video_path, inputdict={'-r': str(fps)}, outputdict={'-vcodec': 'libx264'})
